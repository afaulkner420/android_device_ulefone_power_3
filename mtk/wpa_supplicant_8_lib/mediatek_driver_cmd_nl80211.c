...
static void wpa_driver_send_hang_msg(struct wpa_driver_nl80211_data __unused *drv)
{
    drv_errors++;
...
*
*/
static int wpa_driver_nl80211_send_msg(void *priv, const u8 __unused *msg_in, int msg_in_len,
    u8 __unused *msg_out, int __unused *msg_out_len)
{
    struct i802_bss *bss = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    int ret = 0;
...
static int print_sta_statistics(struct wpa_supplicant __unused *wpa_s, struct wpa_driver_sta_statistics_s *sta_stats,
              unsigned long __unused mask, char *buf, size_t buflen)
{
    size_t i;
    int ret;
    char *pos, *end;
...
static int wpa_driver_get_sta_link_detect(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(pos, end - pos,
                "\nFail=%" PRIu64 " Retry=%" PRIu64 " MulRetry=%" PRIu64 " ACKFail=%" PRIu64 " FCSErr=%" PRIu64
                " TxOk=%" PRIu64 " RxOk=%" PRIu64 " Rst:Reason=%u Time=%" PRIu64 " RoamFail:Times=%u Time=%" PRIu64
                " TxDone:IsARP=%d ArrDrv=%u EnQ=%u DeQ=%u LeaveDrv=%d Curr=%d CurrTime=%" PRIu64 "\n",
                ...
                sta_link_detect.de_que_tick, sta_link_detect.leave_drv_tick, sta_link_detect.curr_tick,
                sta_link_detect.curr_time);
...
static int p2p_ctrl_iface_set_opps(struct wpa_supplicant *wpa_s, char *cmd, char __unused *buf, size_t __unused buflen)
{
    char *str = NULL;
    ...
    *str ++ = '\0';
    ...
    if (ssid) {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE set_opps CTWin=%d "MACSTR" SSID(%d)%s\n",
            CTWin, MAC2STR(addr), (int) ssid_len, ssid);
        os_free(ssid);
    }
    else {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE set_opps CTWin=%d "MACSTR" SSID(%d)\n",
            CTWin, MAC2STR(addr), (int) ssid_len);
    }
...
static int p2p_ctrl_iface_set_power_save(struct wpa_supplicant *wpa_s, char *cmd, char __unused *buf,
                                          size_t __unused buflen)
{
    char *str = NULL;
    int len = 0;
    int value = 0;
...
static int p2p_ctrl_iface_set_sleep(struct wpa_supplicant *wpa_s, char *cmd, char __unused *buf,
                                          size_t __unused buflen)
{
    char *str = NULL;
    u8 addr[ETH_ALEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    ...
    *str ++ = '\0';
    ...
    if (ssid) {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE set_sleep "MACSTR" SSID(%d)%s\n",
            MAC2STR(addr), (int) ssid_len, ssid);
        os_free(ssid);
    }
    else {
        wpa_printf(MSG_DEBUG, "CTRL_IFACE set_sleep "MACSTR" SSID(%d)\n",
            MAC2STR(addr), (int) ssid_len);
    }
...
static int p2p_set_capability(struct wpa_supplicant *wpa_s, char *cmd, char __unused *buf, size_t __unused buflen)
{
    int ret = 0;
...
static void wpa_driver_nl80211_scan_loading_timeout(void *eloop_ctx, void __unused *timeout_ctx)
{
                  ...
                  size_t buf_len )
{
    ...
    if (os_strncasecmp(cmd, "POWERMODE ", 10) == 0) {
        ...
        ret = linux_set_iface_flags(drv->global->ioctl_sock, drv->first_bss->ifname, 1);
        if (ret > 0) {
        ...
        if (wpa_s->drv_priv) {
		    ...
		    */
            ...
            wpa_printf(MSG_DEBUG, "mcc = %d wpa_s drv_flags 0x%x", mcc, (unsigned int) wpa_s->drv_flags);
            ret = 0;
        }
        ...
        *value++ = '\0';
        ...
        wpa_driver_nl80211_testmode(priv, (const u8*)&params, sizeof(params));
        ...
        ret = wpa_driver_nl80211_testmode(priv, (const u8*)&params, sizeof(params));
    } else {
        u8 buffer[100];
...
int wpa_driver_get_p2p_noa(void *priv, u8 __unused *buf, size_t __unused len)
{
    struct i802_bss *bss = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
...
int wpa_driver_set_p2p_ps(void *priv, int __unused legacy_ps, int __unused opp_ps, int __unused ctwindow)
{
    struct i802_bss *bss = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
...
int wpa_driver_set_ap_wps_p2p_ie(void *priv, const struct wpabuf __unused *beacon,
                 const struct wpabuf __unused *proberesp,
                 const struct wpabuf __unused *assocresp)
{
    struct i802_bss *bss = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
